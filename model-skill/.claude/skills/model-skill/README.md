...
> 适用工具：**Claude Code**（`.claude/skills/`）
...
在 Claude Code 对话框中输入以下任一表达，即可激活 model-skill：
...
**Q：推板时在 Claude Code（PowerShell 环境）里运行 bat 脚本失败？**

`.evospec/scripts/push_to_board.bat` 仅适合在 `cmd.exe` 手动双击运行。
Claude Code 在 PowerShell 中推板应直接使用 `references/us-board-deploy.md` 中的 adb 手动命令。

**Q：adb shell 中杀进程命令怎么写？**

使用单引号防止 PowerShell 展开 `$()`，并用 `killall`（板端 BusyBox 无 pkill）：
...
**Q：只改了代码，不需要推整个资源目录，怎么告诉 skill？**

在推板时说"只推二进制"，skill 会使用 `artifacts.main_bin` 而不是 `artifacts.awtkhmi_dir`。

**Q：如何修改 skill 行为？**

- **参数类**（服务器、路径等）：改 `module.config.yaml`
- **流程类**（步骤顺序、输出格式等）：改对应 `references/us-*.md`
- **约束类**（规则增删改）：改 `.evospec/rules/`
- **入口分发逻辑**：改 `SKILL.md`（谨慎修改）
