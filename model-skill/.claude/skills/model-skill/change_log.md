...
- **改动文件**：
  - `references/us-bug-fix.md` — 新增步骤 -1 和步骤 0-PRD（同步应用于 cluster-app 和 update-service）
- **会话摘要**：用户要求每次修 bug 时先查 bug-log 并读取 PRD，手动触发 skill-refiner 完成沉淀。

## 2026-05-12 新增跨模块 skill 同步规则，禁止覆盖模块特有编译/推板配置

- **问题**：将 cluster-app 的 us-build.md 和 us-board-deploy.md 直接覆盖到 update-service，导致 update-service 模块特有的编译命令和推板步骤被错误替换。
- **纠正**：跨模块同步时只能同步通用文件（us-git-submit.md、change_log.md），禁止覆盖 us-build.md 和 us-board-deploy.md。
- **根因分类**：flow
- **WHY**：各模块的编译服务器路径、工具链、推板进程名、写保护方式各不相同，直接覆盖会破坏目标模块的正确流程。
- **改动文件**：
  - `SKILL.md` — 新增"跨模块 skill 同步规则"章节，明确可同步和禁止覆盖的文件清单
- **会话摘要**：用户发现 update-service 的 us-build 和 us-board-deploy 被错误覆盖，要求还原并补充同步规则。
